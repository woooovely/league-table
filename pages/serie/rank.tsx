...
const SerieRank = ({ table, competition, season }: TableDataProps) => {
    return (
        <div>
            <Head>
                    ...
                    {table.map((item: Team) => (
                        <S.ListTr key={item.id}>
                            ...
                            <S.ListTd>
                                ...
                                <S.TeamName>{serieTeamName[item.team.name]}</S.TeamName>
                            </S.ListTd>
...
export const getServerSideProps = async () => {
    try {
        const response = await axios.get("http://localhost:3000/api/serie/rank", {
            headers: {
