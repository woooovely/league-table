...
import { nationality, seriePlayerName, serieTeamName } from "@/constants/constants";
        ...
        {scorers.map((item: Scorer, index: number) => (
            <S.Container key={item.player.id}>
                <S.Table>
                    ...
                    <S.Player>{seriePlayerName[item.player.name]}</S.Player>
                    <S.Goals>{item.goals}</S.Goals>
                    <S.TeamContainer>
                        <S.TeamLogo src={item.team.crest} alt="팀 로고" />
                        {serieTeamName[item.team.name]}
                    </S.TeamContainer>
...
export const getServerSideProps = async () => {
  try {
    const response = await axios.get("http://localhost:3000/api/serie/scorer", {
      headers: {
